...
export class TasksService {
    ...
    async regularizeLocks() {
       try {
            // desbloqueos BG
            const bgLocks = await this.lockRepository.find({where: { end_date_bg_lock: LessThan(new Date()), is_bg_lock: true }});
            for await(let bg of bgLocks){
        ...
        try {
            // desbloqueos WOE
            const woeLocks = await this.lockRepository.find({where: { end_date_woe_lock: LessThan(new Date()), is_woe_lock: true }});
            for await(let woe of woeLocks){
        ...
        try {
            // desbloqueos BAN
            const bans = await this.lockRepository.find({where: { end_date_ban: LessThan(new Date()), is_ban: true }});
            for await(let ban of bans){
